...
- ディレクトリ名が `.{Codex,codex,cursor,aider,codeium,copilot}` のパターンに一致
...
- **skills/ の構造維持**: `.github/skills` へのシンボリックリンクを優先し、
  シンボリックリンクが使えない環境では
  `.Codex/hooks/skills-setup.sh` などの同期スクリプト利用を提案
...
- 既存のskillsとhooks
- `.githooks/` の構造と運用有無
- `.Codex/` `.codex/` のトップレベル構造（設定ファイルとsymlink）
...
git diff HEAD template/main -- .codex/
git diff HEAD template/main -- .Codex/skills/
...
# yellow-seed/template の .Codex/skills/new-skill/SKILL.md を追加
git show template/main:.Codex/skills/new-skill/SKILL.md > .Codex/skills/new-skill/SKILL.md
...
- `.Codex/skills/template-sync/SKILL.md` (新規スキル)
