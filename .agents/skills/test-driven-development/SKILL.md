...
- テストを実行して失敗を確認
  ...
  docker compose exec tool-sidecar bats tests/

  # 特定のテストファイルのみ実行
  docker compose exec tool-sidecar bats tests/example.bats
  ```
...
- Lintで問題がある場合には、コードフォーマットを適用する、個別に修正するなどして対応する
  ...
  docker compose exec tool-sidecar shfmt -d -i 2 .

  # Shell Script Formatting (apply)
  docker compose exec tool-sidecar shfmt -i 2 -w .
  ```
...
- GitHub Actionsワークフローを修正した場合はActionLintを実行
  ...
  docker compose exec tool-sidecar actionlint
  ```
...
- Pull Requestを作成（詳細は [.Codex/skills/pull-request/SKILL.md](../pull-request/SKILL.md) を参照）
