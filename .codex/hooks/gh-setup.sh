...
# This script installs gh CLI when running in Codex
...
if [ "$CODEX_REMOTE" != "true" ]; then
  log "Not a remote Codex session, skipping gh setup"
  exit 0
fi

log "Remote Codex session detected, checking gh CLI..."
...
if [ -x "$LOCAL_BIN/gh" ]; then
  ...
  if [[ ":$PATH:" != *":$LOCAL_BIN:"* ]]; then
    ...
    # Persist to CODEX_ENV_FILE if available
...
# Persist PATH to CODEX_ENV_FILE if available
if [ -n "$CODEX_ENV_FILE" ]; then
  echo "export PATH=\"$LOCAL_BIN:\$PATH\"" >>"$CODEX_ENV_FILE"
