...
OpenSpec導入により、`openspec-propose` / `openspec-explore` / `openspec-apply-change` / `openspec-archive-change` も利用できます。

**注意**: `.claude/skills/`は`.agents/skills/`へのシンボリックリンクです。シンボリックリンクが機能しない環境（Windows管理者権限なしなど）では、`.claude/hooks/skills-setup.sh`を実行してください。
...
このプロジェクトでは、ランタイムバージョン管理に [mise](https://mise.jdx.dev/) を使用しています。
`mise` 自体のインストールは以下のいずれかの方法で行ってください。

**スクリプトで個別インストール:**

```bash
bash scripts/installers/mise.sh
```
...
`bats` テストを実行する場合は、`bats` コマンドがPATH上にあることを確認してください。
...
コミット前にローカルチェックを実行するため、以下のスクリプトでGit hooksを有効化できます。
